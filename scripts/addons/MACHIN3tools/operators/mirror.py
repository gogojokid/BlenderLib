...
def draw_mirror(op):
    def draw(self, context):
        ...
        if not op.remove:
            ...
            row.label(text=f"游标: {op.cursor}")

            if op.cursor and op.cursor_empty:
                row.separator(factor=1)

                row.label(text="", icon='EVENT_E')
                row.label(text=f"使用现有: {op.use_existing_cursor}")
        ...
        if op.mirror_mods:
            ...
            row.label(text=f"删除镜像: {op.remove}")
            ...
            if op.remove and op.misaligned:
                if not op.misaligned['isallmisaligned']:
                    ...
                    row.label(text=f"切换镜像物体: {op.use_misalign}")
...
class Mirror(bpy.types.Operator):
    ...
    def draw_HUD(self, context):
        if not self.passthrough:
            ...
            title = '删除' if self.remove else '镜像'
            ...
            if self.remove and self.misaligned and self.use_misalign:
                name = '空游标' if self.use_misalign and self.mirror_obj.type == 'EMPTY' else self.mirror_obj.name if self.use_misalign else '无'
                ...
                #CHANGE
                    title, color = ('新建游标', green) if self.cursor and not self.use_existing_cursor else ('现有游标', blue) if self.cursor else (self.active.name, yellow)
            ...
            #CHANGE
            if 'negative' in title.lower() or 'positive' in title.lower():
                t,axis = title.split()
                k = {'Negative': '-',
                    'Positive':'+',
                    }
