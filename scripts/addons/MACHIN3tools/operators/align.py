...
def draw_align_relative_status(op):
    def draw(self, context):
        ...
        row.label(text=f"实例: {op.instance}")
...
class AlignRelative(bpy.types.Operator):
    ...
    def draw_HUD(self, args):
        ...
        # CHANGE
        draw_label(context, title='实例' if self.instance else '复制', coords=Vector((self.HUD_x, self.HUD_y)), center=False, color=green if self.instance else blue)
